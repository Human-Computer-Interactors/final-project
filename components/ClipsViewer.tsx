...
const ClipsViewer: FunctionComponent<ClipsViewerProps> = ({ currentTrack, selectedMode, playing, playTimer, mixId, pause, skipTo }) => {
  ...
  const trackOffsets = useMemo<number[]>(
    () => prefixSums(trackSegments.map(({ start, end }) => (end - start) * ONE_SECOND_WIDTH)),
    [trackSegments]
  );

  // Find the current track segment based on scroll position
  const getScrollLocation = (slop: number = 0) => {
    let index = trackOffsets.findIndex((pos, i) => (
      ...
      <Animated.FlatList
        ...
        renderItem={({ item, index }) => (
          <View style={[styles.trackSegment, {
            ...
            backgroundColor: !selectedMode || currentTrack === index
